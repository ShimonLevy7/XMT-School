...
const DefaultAvatarUrl: string =
	'https://cdn.discordapp.com/attachments/836004673299021824/1107722671850004570/avatar.png';
...
export const getCurrentUserAsync = async function () {
	let user: string | IUserResult = await Database.getByKeyAsync('user', [
		CurrentUserToken,
	]);

	if (
		typeof user === 'string' ||
		user.message.messageType === ServerMessageTypes.Error
	)
		return DefaultUser;
...
export const matchesMinUserType = function (
	level: number,
	levels: number | object | null
) {
	if (levels === null) return true;

	if (typeof levels === 'number' && levels >= level) return true;

	if (Array.isArray(levels) && levels.includes(level)) return true;
...
export const isLoggedInAsync = async function () {
	...
	if (currentUser.type === UserTypes.Guest) return false;
